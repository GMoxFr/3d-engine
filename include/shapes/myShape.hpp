...
public:
	myShape(myColor const &clr, double diffuse = 1.0, double fresnel = 1.0, double reflection = 0.0, double refraction = 0.0) : color(clr),
																																hasTexture(false),
																																diffuse(diffuse),
																																fresnel(fresnel),
	...
	bool setTexture(std::string const &filename)
	{
		hasTexture = true;
	...
	bool setBumpMap(std::string const &filename)
	{
		hasBumpMap = true;
		bumpMap = std::make_unique<myTexture>(filename);
		return !bumpMap->getError();
	}
	...
	bool hasTextureError() const { return hasTexture && texture->getError(); }
	bool hasBumpMapError() const { return hasBumpMap && bumpMap->getError(); }
